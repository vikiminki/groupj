...
def ask_input(msg, type=str, allow_empty=False):
    ...
    if answer.lower() == "b" or answer.lower() == "back":
        raise BackToMenu()
    if type == str:
        return answer
    elif type == int:
...
def ask_players(num_players=None, min_players=3, max_players=8):
    ...
    if num_players is None:
        ...
        num_players = ask_input(
            'Enter number of players or [B]ack to get back to menu:', type=int)
    ...
    for n in range(1, num_players+1):
        while True:
            name = ask_input('Enter name of "Player {} or [B]ack to get back to menu":'.format(n))
            if name in names:
                print('Please enter a unique name, try again! or [B]ack to get back to menu')
            else:
                names.append(name)
                break
        ...
        while not ai_difficulty in ai_types:
            ai_difficulty = ask_input(
                'Enter AI difficulty (easy/medium/hard or leave empty if human or [B]ack to get back to menu):',
                allow_empty=True
            ).lower()
